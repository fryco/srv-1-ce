/*
 * (C) Copyright 2005-2007 Surveyor Corporation
...
void initialization (JPEG_ENCODER_STRUCTURE *jpeg, UINT32 image_format, UINT32 image_width, UINT32 image_height)
{
    ...
    if (image_format == JPEG_FOUR_ZERO_ZERO || image_format == JPEG_FOUR_FOUR_FOUR)
    {
        ...
        if (image_format == JPEG_FOUR_ZERO_ZERO)
        {
            bytes_per_pixel = 1;
    ...
    else
    {
        ...
        if (image_format == JPEG_FOUR_TWO_ZERO)
        {
...
UINT8* jpeg_image_encode (UINT8 *input_ptr,UINT8 *output_ptr, UINT32 quality_factor, JPEGPixelFormat image_format, UINT32 image_width, UINT32 image_height)
{
    UINT16 i, j;
...
UINT8* encodeMCU (JPEG_ENCODER_STRUCTURE *jpeg_encoder_structure, UINT32 image_format, UINT8 *output_ptr)
{
    ...
    if (image_format == JPEG_FOUR_TWO_ZERO || image_format == JPEG_FOUR_TWO_TWO)
    {
        levelshift (Y2);
        DCT (Y2);
    ...
    *output_ptr++ = number_of_components;
    ...
    else
    {
        *output_ptr++ = 0x01;
        ...
        if (image_format == JPEG_FOUR_TWO_ZERO)
            *output_ptr++ = 0x22;
        else if (image_format == JPEG_FOUR_TWO_TWO)
            *output_ptr++ = 0x21;
        else
            *output_ptr++ = 0x11;
